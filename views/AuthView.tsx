...
const GoogleLogo = () => (
  <svg viewBox="0 0 24 24" width="16" height="16" className="shrink-0">
    <path
      fill="#4285F4"
      d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-7.98z"
    />
    <path
      fill="#34A853"
...
export function AuthView(): React.ReactElement {
  ...
  if (app.isSignedIn) {
    ...
    return (
      ...
      >
        <div className="flex-1 min-h-0 w-full min-w-0 flex flex-col overflow-hidden px-3 sm:px-4 md:px-6 max-w-4xl mx-auto">
              ...
              >
                <div
                  ...
                  style={{
                    ...
                    scrollPaddingTop: (CYLINDER_VIEW_HEIGHT - CYLINDER_SLOT_HEIGHT) / 2,
                  }}
                >
                  <div
                    style={{
                      height: (CYLINDER_VIEW_HEIGHT - CYLINDER_SLOT_HEIGHT) / 2,
                      flexShrink: 0,
                    }}
                    aria-hidden
                  />
                  {[0, 1, 2, 3].map((index) => {
                    const slotTop =
                      (CYLINDER_VIEW_HEIGHT - CYLINDER_SLOT_HEIGHT) / 2 +
                      index * CYLINDER_SLOT_HEIGHT;
                        ...
                        >
                          {index === 0 && (
                            <>
                              ...
                              {lesson?.scheduleItemId && lesson.scheduleDate ? (
                                <button
                                  type="button"
                                  onClick={() =>
                                    app.openScheduleToItem(
                                      lesson.scheduleItemId!,
                                      lesson.scheduleDate!
                                    )
                                  }
                                  className="mt-0.5 w-full text-left rounded-lg -mx-0.5 px-0.5 py-1 hover:bg-slate-100/80 dark:hover:bg-slate-800/60 transition-colors"
                                >
                                  ...
                                  <p className="text-[9px] font-semibold text-sky-600 dark:text-sky-300 mt-0.5">
                                    Open in Schedule →
                                  </p>
                                </button>
                              ) : (
                                <>
                                  ...
                                  <p className="text-[10px] text-slate-500 dark:text-slate-400 mt-0.5">
                                    {lesson ? lesson.timeLabel : 'Add a block in Schedule.'}
                                  </p>
                                </>
                              )}
                              <button
                                ...
                                className="mt-1 text-[10px] font-semibold text-indigo-600 dark:text-indigo-300 underline underline-offset-2"
                              >
                                Open Grade
                              </button>
                            </>
                            ...
                            <>
                              ...
                              <p className="text-[11px] mt-0.5 text-slate-700 dark:text-slate-200">
                                {app.homeSummary.assignmentsToGrade > 0
                                  ? (app.homeSummary.assignmentsPrimaryLabel ??
                                    `${app.homeSummary.assignmentsToGrade} submission${app.homeSummary.assignmentsToGrade === 1 ? '' : 's'} waiting to sync`)
                                  : 'No work waiting to sync right now.'}
                              </p>
                              {app.homeSummary.assignmentsSecondaryLabel && (
                              ...
                              <button
                                type="button"
                                onClick={quickActions.find((q) => q.key === 'grade')?.onClick}
                                className="mt-1 text-[10px] font-semibold text-amber-600 dark:text-amber-300 underline underline-offset-2"
                              >
                                Scan, grade &amp; sync
                              </button>
                            </>
                            ...
                            <>
                              ...
                              <p className="text-[11px] mt-0.5 text-slate-700 dark:text-slate-200">
                                {app.homeSummary.parentsToContact > 0
                                  ? (app.homeSummary.parentsLabel ??
                                    `${app.homeSummary.parentsToContact} parent${app.homeSummary.parentsToContact === 1 ? '' : 's'} to contact from your follow‑up list.`)
                              ...
                              <button
                                ...
                                onClick={() => {
                                  ...
                                  quickActions.find((q) => q.key === 'communicate')?.onClick();
                                }}
                                className="mt-1 text-[10px] font-semibold underline underline-offset-2 disabled:opacity-45 disabled:cursor-not-allowed disabled:no-underline text-emerald-600 dark:text-emerald-300 disabled:text-slate-400 dark:disabled:text-slate-500"
                              >
                                {app.isOnline ? 'Send message' : 'Send message (online)'}
                              </button>
                            </>
                          )}
                          {index === 3 && (
                            <>
                              ...
                              <button
                                ...
                                className="mt-1 text-[10px] font-semibold text-sky-600 dark:text-sky-300 underline underline-offset-2"
                              >
                                View schedule
                              </button>
                            </>
                ...
                <div
                  className="pointer-events-none absolute inset-x-0 top-0 h-12 bg-gradient-to-b from-slate-50/95 dark:from-slate-950/95 to-transparent"
                  aria-hidden
                />
                <div
                  className="pointer-events-none absolute inset-x-0 bottom-0 h-12 bg-gradient-to-t from-slate-50/95 dark:from-slate-950/95 to-transparent"
                  aria-hidden
                />
              </div>
              <p className="text-center text-[9px] text-slate-500 dark:text-slate-400 mt-1.5">
                Scroll up/down · pauses on touch
              </p>
            </section>
          </div>
        </div>
  ...
  return (
    <PageWrapper
      ...
      syncStatus={app.syncStatus}
    >
      <div className="flex-1 min-h-0 w-full min-w-0 flex flex-col py-0.5 sm:py-1 gap-0.5 sm:gap-1 overflow-hidden px-3 sm:px-5 md:px-8 max-w-4xl mx-auto">
        ...
        <div className="flex-1 min-h-0 overflow-y-auto custom-scrollbar flex flex-col gap-1 py-0.5">
          ...
          <button
            ...
            aria-label={
              !app.isOnline ? 'Sign in with Google (unavailable offline)' : 'Sign in with Google'
            }
            className="mt-8 w-full h-10 min-h-[40px] rounded-lg border border-slate-200 dark:border-slate-700 text-[10px] font-semibold flex flex-col items-center justify-center gap-0.5 px-3 active:scale-[0.98] transition-all bg-white/95 dark:bg-slate-900/95 text-slate-700 dark:text-slate-200 hover:bg-white dark:hover:bg-slate-800 disabled:opacity-45 disabled:cursor-not-allowed disabled:hover:bg-white/95 dark:disabled:hover:bg-slate-900/95"
          >
            <GoogleLogo />
            <span className="font-black uppercase tracking-[0.14em] text-center">
              Sign in with Google
            </span>
          </button>
          ...
          {app.authMode === 'signin' && (
            ...
            >
              ...
              />
              <span
                className="absolute inset-0 rounded-xl bg-white/0 group-hover:bg-white/20 transition-colors duration-300"
                aria-hidden
              />
              <span className="relative z-10 flex items-center justify-center gap-2">
                ...
                <span className="text-white drop-shadow-sm uppercase tracking-[0.12em] text-sm sm:text-base">
                  Sign up today!
                </span>
              </span>
            </button>
          ...
          {app.authMode === 'signup' && (
            ...
            >
              ...
              <span className="relative z-10 flex items-center justify-center gap-2">
                ...
                <span className="text-white drop-shadow-sm uppercase tracking-[0.12em]">
